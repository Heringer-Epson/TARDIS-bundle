...
class_make_yml = getattr(module_make_yml,'make_yml')
class_simulate_spectra = getattr(module_run_simulations,'simulate_spectra')

##########################  CALLING CLASSES  ###########################

class master(class_input):

    #def __init__(self, flag_make_structure=True, flag_make_yml=True, flag_run_simulation=True, flag_kromer_plot=False, flag_display_interface=False, flag_clean_dirs=False, verbose=True):
    def __init__(self, flag_make_structure=True, flag_make_yml=True, flag_run_simulation=True, flag_display_interface=False, verbose=True):
        ...
        if module_input.__file__.split('/')[-1].split('_v')[0] == 'input_pars_fast':
